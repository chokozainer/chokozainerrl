...
from chokozainerrl.experiments.evaluator import AsyncEvaluator
...
def train_loop(process_idx, env, agent, steps, outdir, counter,
               episodes_counter, training_done,pbar,
               max_episode_len=None, evaluator=None, eval_env=None,
               successful_score=None,checkpoint_freq=None,
               logger=None,log_type=None,
               global_step_hooks=()):
    ...
    try:
        ...
        while True:
            ...
            if done or reset or global_t >= steps or training_done.value:
                ...
                obs = env.reset()
                r = 0
            if checkpoint_freq and global_t % checkpoint_freq == 0:
                dirname = os.path.join(outdir, '{}_checkpoint'.format(global_t))
                agent.save(dirname)
    ...
    def run_func(process_idx):
        ...
        def f():
            train_loop(
                ...
                logger=logger,log_type=log_type)
