...
def run_evaluation_episodes(env, agent, n_steps, n_episodes,
                            max_episode_len=None, logger=None,log_type=None):
    ...
    while not terminate:
        ...
        if reset:
            ...
            if log_type=="full_stream":
                print('evaluation episode %s length:%s R:%s' %(len(scores), episode_len, test_r))
...
class Evaluator(object):
    ...
    def __init__(self,
        ...
        self.log_type = log_type or "full_stream"
    ...
    def evaluate_and_update_max_score(self, t, episodes):
        eval_stats = eval_performance(
            ...
            logger=self.logger,log_type=self.log_type)
        ...
        #add choko
        if self.log_type=='pbar':
            custom_columns = tuple(t[0] for t in self.agent.get_statistics())
            column_names = _basic_columns + custom_columns
            print('\t'.join(column_names))            
            print('\t'.join('{:.6g}'.format(x) for x in values)) 
